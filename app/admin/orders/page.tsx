    ...
    return (
        <div className="p-8 max-w-7xl mx-auto space-y-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Gestión de Pedidos</h1>
                    <p className="text-slate-500 font-medium tracking-tight">Administra tus ventas, historial y reportes financieros.</p>
                </div>
            </div>
                    ...
                    <CardContent className="p-4 flex flex-wrap items-center gap-4">
                        <div className="flex-1 min-w-[240px] relative">
                            ...
                            <input
                                ...
                                className="w-full bg-slate-50 border-none rounded-xl pl-12 pr-4 py-2.5 text-xs font-bold focus:ring-2 focus:ring-black outline-none"
                            />
                        </div>
                        ...
                        <div className="flex items-center gap-3 ml-auto md:ml-0">
                            <Button
                                ...
                                variant="ghost"
                                className="rounded-xl h-9 px-4 text-slate-400 hover:text-slate-900 font-black text-[10px] uppercase tracking-widest gap-2"
                            >
                                <RefreshCcw size={14} />
                                Reiniciar
                            </Button>
                            ...
                            <Button
                                onClick={() => fetchOrders()}
                                disabled={isLoading}
                                className="bg-black text-white hover:bg-slate-800 rounded-xl h-9 px-6 font-black text-[10px] uppercase tracking-widest shadow-lg shadow-black/10 disabled:opacity-50"
                            >
            ...
            <Tabs defaultValue="pending" value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="bg-white p-1 rounded-2xl shadow-sm border border-slate-100 h-14 w-full md:w-auto">
                    <TabsTrigger value="pending" className="rounded-xl px-8 data-[state=active]:bg-orange-50 data-[state=active]:text-orange-600 font-black text-[10px] uppercase tracking-widest h-full">
                        Pendientes
                    </TabsTrigger>
                    <TabsTrigger value="all" className="rounded-xl px-8 data-[state=active]:bg-slate-900 data-[state=active]:text-white font-black text-[10px] uppercase tracking-widest h-full">
                        Historial
                    </TabsTrigger>
                    <TabsTrigger value="reports" className="rounded-xl px-8 data-[state=active]:bg-green-50 data-[state=active]:text-green-600 font-black text-[10px] uppercase tracking-widest h-full">
                        Reportes
                    </TabsTrigger>
                </TabsList>
                    ...
                    <TabsContent value="reports" className="mt-0 space-y-8">
                        {reportData && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <Card className="bg-gradient-to-br from-green-500 to-emerald-600 text-white border-none shadow-lg rounded-[2rem]">
                                    <CardContent className="p-8">
                                        <div className="flex justify-between items-center mb-4">
                                            <p className="font-black text-[10px] uppercase tracking-[0.3em] opacity-80">Ingresos Totales</p>
                                            <ShoppingBag className="h-5 w-5 opacity-40" />
                                        </div>
                                        <h3 className="text-4xl font-black italic tracking-tighter">${reportData.totalRevenue.toFixed(2)}</h3>
                                        <p className="text-[10px] mt-4 font-bold opacity-60">Total Acumulado</p>
                                    </CardContent>
                                </Card>
                                ...
                                <Card className="bg-white border-none shadow-sm rounded-[2rem]">
                                    <CardContent className="p-8">
                                        <div className="flex justify-between items-center mb-4">
                                            <p className="font-black text-[10px] text-slate-400 uppercase tracking-[0.3em]">Ventas Realizadas</p>
                                            <CheckCircle className="h-5 w-5 text-slate-200" />
                                        </div>
                                        <h3 className="text-4xl font-black text-slate-900 italic tracking-tighter">{reportData.saleCount}</h3>
                                        <div className="mt-4 flex items-center gap-2">
                                            <TrendingUp className="h-4 w-4 text-green-500" />
                                            <span className="text-[10px] font-black text-green-500 uppercase tracking-widest">Órdenes Completadas</span>
                                        </div>
                                    </CardContent>
                                </Card>
                            </div>
    ...
    return (
        <div className="space-y-4">
            {orders.map((order: any) => (
                <Card key={order.id} className="overflow-hidden border-none shadow-sm hover:shadow-md transition-all bg-white rounded-[2rem]">
                    <div
                    ...
                    >
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                            <div className="flex items-center gap-4">
                                <div className={`h-12 w-12 rounded-2xl flex items-center justify-center ${order.status === 'PENDING' ? 'bg-orange-50 text-orange-600' :
                                ...
                                <div>
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">PEDIDO {order.status}</p>
                                    <p className="font-black text-slate-900">{order.id.substring(0, 12).toUpperCase()}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="font-bold text-slate-900">{order.user?.name || 'Venta Directa'}</p>
                                </div>
                            </div>
